...
# Root endpoint 
@app.route('/graphic/<res>')
def graphic(res):
    # Define Plot Data 
    res = ast.literal_eval(res)
    labels = [i[0][:10] for i in res]
    data = [i[1] for i in res]
    middle = [sum([i[1] for i in res])//len(res) for _ in range(len(res))]
...
def rate(task,figi,name):
    ...
    if not database.check_name(name):
        ...
        for i in range(len(res)):
            uid2 = str(uuid.uuid4())
            database.inset_data('quots',(uid2,res[i][0], res[i][1], uid))  
        database.db_close()
    else:
        res = database.check_name(name).fetchall()
